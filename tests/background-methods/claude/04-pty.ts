/**
 * Test: Run Claude Code using PTY (Pseudo Terminal)
 *
...
async function main() {
  console.log("=== PTY Background Method (Claude) ===\n")
  ...
  const ptyId = `claude-pty-${Date.now()}`
  ...
  try {
    ...
    const outputFile = "/tmp/claude-pty-output.jsonl"
    const pidFile = "/tmp/claude-pty.pid"
    ...
    // 4. Start claude in PTY (capture PID)
