...
/**
 * Creates an accumulator for streaming content that can be saved to database
 * even if the client disconnects
 */
export interface ContentAccumulator {
  /** Add content to accumulator */
  addContent: (content: string) => void
  /** Add a tool call to accumulator */
  addToolCall: (toolCall: { tool: string; summary: string }) => void
  /** Get accumulated content */
  getContent: () => string
  /** Get accumulated tool calls */
  getToolCalls: () => Array<{ tool: string; summary: string }>
  /** Append error to content */
  addError: (message: string) => void
}

/**
 * Creates a content accumulator for tracking streamed content
 */
export function createContentAccumulator(): ContentAccumulator {
  ...
  return {
    addContent: (text: string) => { content += text },
    addToolCall: (toolCall) => { toolCalls.push(toolCall) },
    getContent: () => content,
